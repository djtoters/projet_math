...
df_anderlecht = df[df["TX_DESCR_FR"] == "Anderlecht"]

# Préparer les données
X = df_anderlecht['CASES_PER_10K_MA'].values.reshape(-1, 1)

# Diviser les données en 70% train et 30% test
X_train, X_test, dates_train, dates_test = train_test_split(X, df_anderlecht['DATE'], test_size=0.3, random_state=425, shuffle=False)
